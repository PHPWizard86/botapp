...
class UserMenu extends InlineMenu {
    ...
    public function fileUpload(Nutgram $bot, string $data) {
        ...
        if( is_null( $site ) || empty( $site[ 'accepted' ] ) || !in_array( $bot->userId() ?? 0, $site[ 'user_ids' ] ) ) {
    ...
    public function fileUploadFile(Nutgram $bot) {
        ...
        $update_menu = function() use ( &$disabled, &$next_data ) {
            ...
            if( is_int( $up ) ) {
                ...
                if( $up === 0 ) {
                    ...
                    $up_text = 'در انتظار آپلود ...';
                    
                } else {
                    
                    $up_text = "{$up}% آپلود شده ...";
        ...
        $dl_fn = function( $downloadTotal, $downloadedBytes, $uploadTotal, $uploadedBytes ) use ( &$start, &$next_data, $update_menu ) {
            ...
            if( $downloadTotal > 0 ) {
                ...
                if( $downloadedBytes < 1048576 ) {
                    ...
                    $next_data[ 'dl' ] = number_format( $downloadedBytes / 1024, 2, '.', '' ) . ' کیلوبایت';
                    
                } else {
                    
                    $next_data[ 'dl' ] = number_format( $downloadedBytes / 1048576, 2, '.', '' ) . ' مگابایت';
        ...
        if( 
            ...
            if( $this->isSocialLink( $text ) ) {
                ...
                $dl_link = $this->getSocialDirectLink( $text, $type == 'a' ? true : false );
    ...
    public function site(Nutgram $bot, string $data) {
        ...
        $site = $this->siteStore
            ...
            ->where( [ "_id", "==", $id ] )
            ->where( [ 'accepted', '=', true ] )
            ->where( [ 'user_ids', 'CONTAINS', $bot->userId() ?? 0 ] )
            ->join( function( $site ) {
        ...
        if( is_null( $site ) ) {
            ...
            if( empty( $samples ) || !isset( $site[ 'dl_host' ] ) ) {
                ...
                $this->addText("⚠️ امکان ارسال یا ویرایش پست در این سایت وجود ندارد.");
    ...
    public function saveField() {
        ...
        if( !isset( $field ) || is_null( $site ) || empty( $site[ 'accepted' ] ) || !in_array( $this->bot->userId() ?? 0, $site[ 'user_ids' ] ) ) {
